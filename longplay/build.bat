...
docker run --rm -it -v "%CD%:/src" vcmi-builder bash -c "cd /src/longplay/builder/build && cmake -S ../../../ -DENABLE_CCACHE=OFF -DENABLE_TEST=OFF -DENABLE_MMAI=OFF -DENABLE_LAUNCHER=OFF -DCMAKE_BUILD_TYPE=Release -DENABLE_STATIC=ON && cmake --build . -j8"
if %errorlevel% neq 0 (
